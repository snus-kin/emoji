...
def get_cheat_sheet(url: str) -> dict:
    ...
    for emj, short_code in emojis.items():
        ...
        if short_code.startswith(':flag_'):
            # We already have a different format for the flat_ shortcodes
            continue

        if '⊛' in short_code:
            # Strange emoji with ⊛ in the shortcode
            continue

        if emj == '\U0001F93E\U0000200D\U00002640\U0000FE0F':
            # This shortcode is simplay wrong
            continue
        ...
        if short_code.startswith('::') and short_code.endswith('::'):
            # Do not allow shortcodes to have double :: at the start and end
            short_code = short_code[1:-1]
